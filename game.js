...
const openUnlockFromCreatureBtn = $id("openUnlockFromCreatureBtn");
const openTrapFromCreatureBtn = $id("openTrapFromCreatureBtn");
const openUnlockFromTrapBtn = $id("openUnlockFromTrapBtn");
const openCreatureFromTrapBtn = $id("openCreatureFromTrapBtn");
...
const CLOUD_AUTH_STORAGE_KEY = "sb-wfsywtfktmthxpgqoyoi-auth-token";
const CLOUD_AUTH_RUNTIME_FILENAME = "supabase-auth-session-v1.json";
...
const audioSystem = {
  ...
  playBombarderImpact(damage = 420, splashRadius = 2.8) {
    const ctx = this.ensure();
    if (!ctx || ctx.state !== "running" || !this.sfxGain || !this.sfxEnabled || !this.shotSfxEnabled) return;

    const now = ctx.currentTime;
    if (now < this.bombImpactCooldownUntil) return;
    this.bombImpactCooldownUntil = now + 0.05;

    const damageNorm = Math.max(120, Math.min(2000, Number(damage) || 420));
    const splashNorm = Math.max(0.8, Math.min(7, Number(splashRadius) || 2.8));
    const intensity = Math.max(0.8, Math.min(3.4, damageNorm / 520 + splashNorm * 0.28));
    const blastDuration = 0.24 + intensity * 0.09;

    const sub = ctx.createOscillator();
    sub.type = "sine";
    sub.frequency.setValueAtTime(84 + intensity * 10 + Math.random() * 4, now);
    sub.frequency.exponentialRampToValueAtTime(Math.max(26, 34 + intensity * 2), now + blastDuration);

    const subGain = ctx.createGain();
    subGain.gain.setValueAtTime(0.0001, now);
    subGain.gain.exponentialRampToValueAtTime(Math.min(0.28, 0.085 + intensity * 0.055), now + 0.01);
    subGain.gain.exponentialRampToValueAtTime(0.0001, now + blastDuration);
    sub.connect(subGain);
    subGain.connect(this.sfxGain);
    sub.start(now);
    sub.stop(now + blastDuration + 0.04);

    const crack = ctx.createOscillator();
    crack.type = "square";
    crack.frequency.setValueAtTime(230 + intensity * 34, now + 0.004);
    crack.frequency.exponentialRampToValueAtTime(62 + intensity * 8, now + blastDuration * 0.56);

    const crackFilter = ctx.createBiquadFilter();
    crackFilter.type = "bandpass";
    crackFilter.Q.value = 1.6;
    crackFilter.frequency.setValueAtTime(520 + intensity * 170, now);

    const crackGain = ctx.createGain();
    crackGain.gain.setValueAtTime(0.0001, now + 0.004);
    crackGain.gain.exponentialRampToValueAtTime(Math.min(0.16, 0.05 + intensity * 0.03), now + 0.016);
    crackGain.gain.exponentialRampToValueAtTime(0.0001, now + blastDuration * 0.62);
    crack.connect(crackFilter);
    crackFilter.connect(crackGain);
    crackGain.connect(this.sfxGain);
    crack.start(now + 0.004);
    crack.stop(now + blastDuration * 0.66 + 0.03);
    ...
    if (noiseBuffer) {
      ...
      debrisHigh.frequency.setValueAtTime(200 + intensity * 80, now);

      const debrisBand = ctx.createBiquadFilter();
      debrisBand.type = "bandpass";
      debrisBand.Q.value = 1.25;
      debrisBand.frequency.setValueAtTime(760 + intensity * 210 + Math.random() * 120, now);

      const debrisGain = ctx.createGain();
      debrisGain.gain.setValueAtTime(0.0001, now);
      debrisGain.gain.exponentialRampToValueAtTime(Math.min(0.2, 0.07 + intensity * 0.04), now + 0.006);
      debrisGain.gain.exponentialRampToValueAtTime(0.0001, now + 0.13 + intensity * 0.04);

      debris.connect(debrisHigh);
      debrisHigh.connect(debrisBand);
      debrisBand.connect(debrisGain);
      debrisGain.connect(this.sfxGain);
      debris.start(now);
      debris.stop(now + 0.17 + intensity * 0.05);
    }
  },
...
let creatureCardPortraitCamera = null;
let creatureCardPortraitSubject = null;
...
function applyLevelLightingTheme(level) {
  ...
  if (marsLevel) {
    scene.background.set("#452716");
    scene.fog.color.set("#9b5621");
    scene.fog.density = 0.013;
    renderer.toneMappingExposure = 0.9;

    hemiLight.color.set("#c06a2f");
    hemiLight.groundColor.set("#2d180f");
    hemiLight.intensity = 0.58;

    sunLight.color.set("#d07a39");
    sunLight.intensity = 0.96;
    sunLight.position.set(-16, 28, -10);

    tealRim.color.set("#b0632b");
    tealRim.intensity = 0.4;
    tealRim.position.set(-22, 9, -14);

    emberRim.color.set("#bd6a2f");
    emberRim.intensity = 0.4;
    emberRim.position.set(23, 8, 15);

    prismKeyLight.color.set("#bf6f34");
    prismKeyLight.intensity = 0.34;
    prismKeyLight.position.set(-4, 13, -8);

    prismFillLight.color.set("#9a5127");
    prismFillLight.intensity = 0.28;
...
function applyViewportMapVisibilityTuning() {
  ...
  const portraitFactor = getPortraitViewportTuningFactor();
  const baseFogDensity = marsLevel ? 0.013 : emberLevel ? 0.012 : moonLevel ? 0.01 : 0.012;
  const baseExposure = marsLevel ? 0.9 : emberLevel ? 1.0 : moonLevel ? 0.96 : 1.08;
  const baseVignette = marsLevel ? 0.54 : emberLevel ? 0.5 : moonLevel ? 0.56 : 0.48;
  const baseShadowWash = marsLevel ? 0.26 : emberLevel ? 0.26 : moonLevel ? 0.26 : 0.2;
...
function buildMap() {
  ...
  const base = new THREE.Mesh(
    ...
    new THREE.MeshPhysicalMaterial({
      color: marsLevel ? "#553220" : emberLevel ? "#151824" : moonLevel ? "#121620" : "#0a1624",
      roughness: 0.82,
      metalness: 0.18,
      clearcoat: 0.3,
      clearcoatRoughness: 0.48,
      emissive: marsLevel ? "#3a2012" : emberLevel ? "#0d101b" : moonLevel ? "#0b0f18" : "#050e19",
      emissiveIntensity: marsLevel ? 0.48 : emberLevel ? 0.56 : moonLevel ? 0.58 : 0.66,
  ...
  const deck = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({
      color: marsLevel ? "#684028" : emberLevel ? "#1f2436" : moonLevel ? "#1a202d" : "#102538",
      roughness: 0.72,
      metalness: 0.26,
      emissive: marsLevel ? "#3a2011" : emberLevel ? "#141a2a" : moonLevel ? "#101722" : "#08192a",
  ...
  const vignetteMaterial = new THREE.MeshBasicMaterial({
    color: marsLevel ? "#24150c" : emberLevel ? "#080812" : moonLevel ? "#04070e" : "#01060c",
    transparent: true,
  ...
  const shadowWashMaterial = new THREE.MeshBasicMaterial({
    color: marsLevel ? "#332015" : emberLevel ? "#0a0b15" : moonLevel ? "#02050a" : "#01050a",
    transparent: true,
    opacity: marsLevel ? 0.26 : emberLevel ? 0.26 : moonLevel ? 0.26 : 0.2,
    depthWrite: false,
  });
  ...
  const mapLightA = new THREE.PointLight(
    marsLevel ? 0xa55f2b : emberLevel ? 0x6aaeff : moonLevel ? 0x8ab5ff : 0x58c6ff,
    marsLevel ? 0.22 : emberLevel ? 0.42 : moonLevel ? 0.38 : 0.46,
    boardWidth * 1.4,
    2
  );
  ...
  const mapLightB = new THREE.PointLight(
    marsLevel ? 0xc9793f : emberLevel ? 0xff8164 : moonLevel ? 0x8aa0d8 : 0xff9f57,
    marsLevel ? 0.2 : emberLevel ? 0.32 : moonLevel ? 0.3 : 0.38,
    boardWidth * 1.4,
    2
  );
  ...
  const mapLightC = new THREE.PointLight(
    marsLevel ? 0x9f5a2d : emberLevel ? 0xb493ff : moonLevel ? 0xd4e0ff : 0x63f2d3,
    marsLevel ? 0.12 : emberLevel ? 0.26 : moonLevel ? 0.22 : 0.28,
    boardWidth * 1.2,
    2
  );
  ...
  for (let y = 0; y < ROWS; y += 1) {
    for (let x = 0; x < COLS; x += 1) {
      ...
      if (marsLevel) {
        if (onPath) tileColor.setHSL(0.068 + n * 0.012, 0.62, 0.24 + n * 0.04 - edgeBlend * 0.04);
        else tileColor.setHSL(0.062 + duneRidge * 0.022 + n * 0.012, 0.54, 0.19 + n * 0.06 + duneRidge * 0.06 - edgeBlend * 0.07);
      } else if (emberLevel) {
      ...
      if (onPath) {
        const inlay = new THREE.Mesh(
          ...
          new THREE.MeshPhysicalMaterial({
            color: marsLevel ? "#ffd8b4" : emberLevel ? "#ffd4c8" : moonLevel ? "#d7e4ff" : "#ffd5a0",
            emissive: marsLevel ? "#c2763f" : emberLevel ? "#cf5f6f" : moonLevel ? "#8ea6d8" : "#ff9750",
        ...
        if (panelSeed > 0.72) {
          const panel = new THREE.Mesh(
            ...
            new THREE.MeshStandardMaterial({
              color: marsLevel ? "#7d563f" : emberLevel ? "#3d3048" : moonLevel ? "#3a4457" : "#294a63",
              emissive: marsLevel ? "#3b271a" : emberLevel ? "#1d1428" : moonLevel ? "#1a2230" : "#0f2638",
...
function buildPathRails() {
  ...
  const railMaterial = new THREE.MeshPhysicalMaterial({
    color: marsLevel ? "#a96f46" : emberLevel ? "#d7c4ff" : moonLevel ? "#c8d8ff" : "#ffb873",
    emissive: marsLevel ? "#5a321a" : emberLevel ? "#6b4ca8" : moonLevel ? "#5e78a8" : "#8c3714",
    emissiveIntensity: marsLevel ? 0.42 : emberLevel ? 0.82 : moonLevel ? 0.9 : 1.12,
  ...
  const laneMaterial = new THREE.MeshPhysicalMaterial({
    color: marsLevel ? "#7a482b" : emberLevel ? "#3d3258" : moonLevel ? "#3c4f73" : "#874822",
    emissive: marsLevel ? "#472614" : emberLevel ? "#201838" : moonLevel ? "#1f2d47" : "#5f260f",
    emissiveIntensity: marsLevel ? 0.42 : emberLevel ? 0.72 : moonLevel ? 0.74 : 0.94,
  ...
  for (const segment of pathSegments) {
    ...
    const laneGlow = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({
        color: marsLevel ? "#c27a48" : emberLevel ? "#a8bbff" : moonLevel ? "#b9d0ff" : "#ffb870",
        transparent: true,
        opacity: marsLevel ? 0.16 : emberLevel ? 0.26 : moonLevel ? 0.24 : 0.3,
        depthWrite: false,
      })
    );
...
function buildPathPulseOrbs() {
  ...
  const orbMaterial = new THREE.MeshStandardMaterial({
    color: marsLevel ? "#c5814e" : emberLevel ? "#d6dcff" : moonLevel ? "#d6e3ff" : "#ffe7ba",
    emissive: marsLevel ? "#a0612c" : emberLevel ? "#8b6bff" : moonLevel ? "#92aee8" : "#ffb169",
    emissiveIntensity: marsLevel ? 0.48 : emberLevel ? 1.06 : moonLevel ? 0.95 : 1.1,
...
function buildSkyBits(boardWidth, boardHeight) {
  ...
  const starMaterial = new THREE.PointsMaterial({
    color: marsLevel ? "#b06a3a" : emberLevel ? "#c6d7ff" : moonLevel ? "#d6e7ff" : "#a8dcff",
    size: marsLevel ? 0.38 : emberLevel ? 0.28 : moonLevel ? 0.24 : 0.22,
    transparent: true,
    opacity: marsLevel ? 0.34 : emberLevel ? 0.34 : moonLevel ? 0.42 : 0.34,
    depthWrite: false,
...
function buildPerimeterBeacons(boardWidth, boardHeight) {
  ...
  for (const [x, z] of positions) {
    const post = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({
        color: marsLevel ? "#5a3722" : emberLevel ? "#4a3850" : moonLevel ? "#465270" : "#355670",
        roughness: 0.52,
    ...
    const lamp = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({
        color: marsLevel ? "#b1744b" : emberLevel ? "#dccbff" : moonLevel ? "#d2e4ff" : "#a2ffe9",
        emissive: marsLevel ? "#955628" : emberLevel ? "#9072ff" : moonLevel ? "#8eaee8" : "#63f5d4",
        emissiveIntensity: marsLevel ? 0.48 : emberLevel ? 1.08 : moonLevel ? 1.05 : 1.2,
...
function getRuntimeCloudAuthSessionPath() {
  const bridge = getRuntimeProgressBridge();
  if (!bridge || !bridge.pathApi || !bridge.directoryPath) return "";
  try {
    return bridge.pathApi.join(bridge.directoryPath, CLOUD_AUTH_RUNTIME_FILENAME);
  } catch (_) {
    return "";
  }
}

function readCloudAuthRuntimeSessionSnapshot() {
  const sessionPath = getRuntimeCloudAuthSessionPath();
  if (!sessionPath) return "";
  return readRuntimeProgressRawFile(sessionPath);
}

function writeCloudAuthRuntimeSessionSnapshot(rawValue) {
  const serialized = typeof rawValue === "string" ? rawValue : "";
  if (!serialized) return false;
  const bridge = getRuntimeProgressBridge();
  const sessionPath = getRuntimeCloudAuthSessionPath();
  if (!bridge || !sessionPath) return false;
  try {
    bridge.fsApi.mkdirSync(bridge.directoryPath, { recursive: true });
  } catch (_) {
    return false;
  }
  return writeRuntimeProgressRawFile(sessionPath, serialized);
}

function clearCloudAuthRuntimeSessionSnapshot() {
  const bridge = getRuntimeProgressBridge();
  const sessionPath = getRuntimeCloudAuthSessionPath();
  if (!bridge || !sessionPath) return;
  try {
    if (bridge.fsApi.existsSync(sessionPath)) bridge.fsApi.unlinkSync(sessionPath);
  } catch (_) {}
}
...
function createCloudAuthStorageAdapter() {
  return {
      ...
      if (storage) {
        try {
          ...
          if (typeof value === "string") {
            if (safeKey === CLOUD_AUTH_STORAGE_KEY && value) writeCloudAuthRuntimeSessionSnapshot(value);
            return value;
          }
        } catch (_) {}
      }
      if (safeKey !== CLOUD_AUTH_STORAGE_KEY) return null;
      const runtimeValue = readCloudAuthRuntimeSessionSnapshot();
      if (!runtimeValue) return null;
      if (storage) {
        try {
          storage.setItem(safeKey, runtimeValue);
        } catch (_) {}
      }
      return runtimeValue;
    },
    setItem(key, value) {
      ...
      if (storage) {
        try {
          storage.setItem(safeKey, serialized);
        } catch (_) {}
      }
      if (safeKey !== CLOUD_AUTH_STORAGE_KEY) return;
      if (serialized) writeCloudAuthRuntimeSessionSnapshot(serialized);
      else clearCloudAuthRuntimeSessionSnapshot();
    },
    removeItem(key) {
      const safeKey = String(key || "");
      const storage = getProgressStorage();
      ...
      if (safeKey === CLOUD_AUTH_STORAGE_KEY) clearCloudAuthRuntimeSessionSnapshot();
    },
  };
}
...
function isLikelyFreshDefaultProgress(data) {
  ...
  return (!Number.isFinite(shards) || Math.floor(shards) <= 0) && (!Number.isFinite(level) || Math.floor(level) <= 1);
}
...
function updateAccountCloudUi() {
  ...
  if (cloudAuth.profileTableMissing) {
    menuAccountNoteEl.textContent = `${ACCOUNT_NOTE_BASE_TEXT} Cloud login is active, but cloud table setup is missing. Run supabase_setup.sql in Supabase SQL Editor.`;
    return;
  }
  if (signedInEmail) {
...
function captureGameProgressSeedForAccountMigration() {
  const maxLoadoutSlots = getCurrentLoadoutSlotLimit();
  const unlockedTowers = new Set(game.unlockedTowers);
  const unlockedSpawnerTowers = new Set(game.unlockedSpawnerTowers);
  let activeLoadout = Array.from(game.activeLoadout)
    .filter((towerTypeId) => {
      if (unlockedTowers.has(towerTypeId)) return true;
      const enemyTypeId = enemyTypeFromSpawnerTowerId(towerTypeId);
      return !!enemyTypeId && unlockedSpawnerTowers.has(enemyTypeId);
    })
    .slice(0, maxLoadoutSlots);
  if (activeLoadout.length === 0) {
    const fallbackLoadout = new Set();
    addDefaultLoadoutEntries(fallbackLoadout, unlockedTowers, unlockedSpawnerTowers, maxLoadoutSlots);
    activeLoadout = Array.from(fallbackLoadout);
  }
  return {
    shards: Math.max(0, Math.floor(game.shards || 0)),
    maxLevelUnlocked: Math.max(1, Math.floor(game.maxLevelUnlocked || 1)),
    maxLoadoutSlots,
    unlockedTowers: Array.from(unlockedTowers),
    activeLoadout,
    enemyKillCounts: normalizeEnemyKillCounts(game.enemyKillCounts),
    unlockedSpawnerTowers: Array.from(unlockedSpawnerTowers).filter((enemyTypeId) =>
      Object.prototype.hasOwnProperty.call(CREATURE_SPAWNER_UNLOCKS, enemyTypeId)
    ),
...
function ensureLocalAccountRecord(email, password = "", options = null) {
  const normalizedEmail = sanitizeAccountUsername(email);
  if (!isEmailAddress(normalizedEmail)) return null;
  const setPasswordOnExisting = !!options?.setPasswordOnExisting;
  const setPasswordOnCreate = options?.setPasswordOnCreate !== false;
  const setDisplayNameOnExisting = !!options?.setDisplayNameOnExisting;
  const requestedDisplayName = sanitizeAccountName(options?.displayName || "", ACCOUNT_DISPLAY_NAME_MAX_LENGTH);
  const displayNameOnCreate =
    requestedDisplayName || deriveAccountDisplayNameFromEmail(normalizedEmail) || "Commander";
  const seedOnCreate =
    options?.seedOnCreate && typeof options.seedOnCreate === "object"
      ? options.seedOnCreate
      : null;
  const passwordHash = password && setPasswordOnCreate ? hashAccountPassword(normalizedEmail, password) : "";

  let account = findAccountByUsername(normalizedEmail);
  if (!account) {
    account = createAccountRecord(displayNameOnCreate, {
      ...(seedOnCreate || {}),
      email: normalizedEmail,
      name: displayNameOnCreate,
      passwordHash,
    });
    progressData.accounts.push(account);
  } else {
    account.email = normalizedEmail;
    if (password && setPasswordOnExisting) {
      account.passwordHash = hashAccountPassword(normalizedEmail, password);
    }
    if (setDisplayNameOnExisting && requestedDisplayName) {
      account.name = requestedDisplayName;
    } else if (
      !sanitizeAccountName(account.name || "", ACCOUNT_DISPLAY_NAME_MAX_LENGTH) ||
      isEmailAddress(account.name || "")
    ) {
      account.name = displayNameOnCreate;
    }
  }
...
function flushQueuedCloudSync(forceStatus = false) {
  ...
  void syncCloudProgressNow(forceStatus);
}
...
async function loadCloudProgressForUser(user, quiet = false, fallbackEmail = "") {
  ...
  try {
    const { data, error } = await cloudAuth.client
      ...
      .select("progress, updated_at, profile_name, email")
      .eq("id", user.id)
      .maybeSingle();
    if (error) {
      if (isCloudProfilesTableMissing(error)) {
    ...
    if (signedInEmail) {
      const signedInAccount =
        findAccountByUsername(signedInEmail) ||
        ensureLocalAccountRecord(signedInEmail, "", {
          displayName: cloudProfileName,
          setDisplayNameOnExisting: !!cloudProfileName,
        });
      if (signedInAccount) {
        signedInAccount.email = signedInEmail;
        if (cloudProfileName) {
          signedInAccount.name = cloudProfileName;
        } else if (!sanitizeAccountName(signedInAccount.name || "", ACCOUNT_DISPLAY_NAME_MAX_LENGTH)) {
          signedInAccount.name = deriveAccountDisplayNameFromEmail(signedInEmail) || "Commander";
        }
        progressData.currentAccountId = signedInAccount.id;
...
async function syncCloudSessionUser(user, quiet = false, fallbackEmail = "") {
  ...
  const account = ensureLocalAccountRecord(signedInEmail, "", {
    seedOnCreate: migrationSeed,
    displayName: preferredDisplayName,
    setDisplayNameOnExisting: !!preferredDisplayName && !isEmailAddress(preferredDisplayName),
  });
  if (account) {
...
async function initializeCloudAuth() {
  ...
  try {
    const authStorage = createCloudAuthStorageAdapter();
  ...
  try {
    ...
    if (sessionUser) {
      ...
      if (cloudAuth.profileTableMissing) {
        ...
        setStatus(`Cloud session restored for ${signedInEmail || "your account"}. Sync is pending.`);
      } else {
...
function deriveAccountDisplayNameFromEmail(email) {
  ...
  return sanitizeAccountName(prettyLocal, ACCOUNT_DISPLAY_NAME_MAX_LENGTH);
}

function getAccountEmail(account) {
  if (!account || typeof account !== "object") return "";
  const explicit = sanitizeAccountUsername(account.email || "");
  if (isEmailAddress(explicit)) return explicit;
  const legacy = sanitizeAccountUsername(account.name || "");
  if (isEmailAddress(legacy)) return legacy;
  return "";
}

function getAccountDisplayName(account, fallback = "Commander") {
  if (!account || typeof account !== "object") return sanitizeAccountName(fallback) || "Commander";
  const rawName = sanitizeAccountName(account.name || "", ACCOUNT_DISPLAY_NAME_MAX_LENGTH);
  if (rawName && !isEmailAddress(rawName)) return rawName;
  const derived = deriveAccountDisplayNameFromEmail(getAccountEmail(account));
...
function findAccountByUsername(email) {
  ...
  return progressData.accounts.find((account) => getAccountUsernameKey(getAccountEmail(account)) === usernameKey) || null;
}
...
function isCloudBackedAccount(account) {
  if (!account || typeof account !== "object") return false;
  return !!getAccountEmail(account);
}
...
function pruneLegacyLocalAccountsForCloudUser(preferredEmail = "") {
  ...
  if (!isEmailAddress(normalizedPreferredEmail)) return false;
  ...
  const preferredAccount =
    cloudAccounts.find((account) => getAccountUsernameKey(getAccountEmail(account)) === preferredKey) || cloudAccounts[0];
  ...
  if (announce) {
    setStatus(
      progressData.requirePasswordOnStartup
        ? "Stay signed in disabled: password required each launch."
        : "Stay signed in enabled on this device."
    );
  }
}
...
function createAccountRecord(name = "Commander", seed = null) {
  ...
  const explicitEmail = sanitizeAccountUsername(seed?.email || "");
  const legacyEmail = sanitizeAccountUsername(seed?.name || "");
  const normalizedEmail = isEmailAddress(explicitEmail) ? explicitEmail : isEmailAddress(legacyEmail) ? legacyEmail : "";
  const rawDisplayName = sanitizeAccountName(seed?.displayName ?? seed?.name ?? name, ACCOUNT_DISPLAY_NAME_MAX_LENGTH);
  const displayName =
    (!rawDisplayName || isEmailAddress(rawDisplayName)
      ? deriveAccountDisplayNameFromEmail(normalizedEmail) || sanitizeAccountName(name, ACCOUNT_DISPLAY_NAME_MAX_LENGTH)
      : rawDisplayName) || "Commander";
...
function applyAccountToGame(account) {
  ...
  game.accountName = getAccountDisplayName(account);
...
function savePlayerProgress() {
  ...
  const currentEmail = getAccountEmail(current) || sanitizeAccountUsername(getCloudSignedInEmail());
  if (currentEmail) current.email = currentEmail;
  current.name = sanitizeAccountName(game.accountName, ACCOUNT_DISPLAY_NAME_MAX_LENGTH)
    || deriveAccountDisplayNameFromEmail(currentEmail)
    || "Commander";
...
function createTowerMesh(towerTypeId, bodyColor, coreColor) {
  ...
  const isStormcoilerBody = towerTypeId === "tesla" || towerTypeId === "deluxeStormcoiler";
  const isDeluxeStormcoilerBody = towerTypeId === "deluxeStormcoiler";

  const base = cast(
    new THREE.Mesh(
      new THREE.CylinderGeometry(
        isStormcoilerBody ? (isDeluxeStormcoilerBody ? 1.08 : 0.96) : 1.26,
        isStormcoilerBody ? (isDeluxeStormcoilerBody ? 1.24 : 1.12) : 1.45,
        isStormcoilerBody ? (isDeluxeStormcoilerBody ? 0.54 : 0.48) : 0.42,
        isStormcoilerBody ? 34 : 26
      ),
      darkMat
    )
  );
  base.position.y = isStormcoilerBody ? 0.26 : 0.21;
  group.add(base);
  ...
  if (isTwoByTwoDeluxeTower) {
    ...
    const footprintDisk = cast(
      new THREE.Mesh(
        new THREE.CylinderGeometry(
          CELL_SIZE * (isDeluxeStormcoilerBody ? 0.58 : 0.68),
          CELL_SIZE * (isDeluxeStormcoilerBody ? 0.62 : 0.74),
          0.14,
          42
        ),
        darkMat
    ...
    const footprintRing = cast(
      new THREE.Mesh(new THREE.TorusGeometry(CELL_SIZE * (isDeluxeStormcoilerBody ? 0.24 : 0.3), 0.028, 10, 40), glowMat)
    );
    ...
    const superHull = cast(
      new THREE.Mesh(
        new THREE.CylinderGeometry(
          CELL_SIZE * (isDeluxeStormcoilerBody ? 0.28 : 0.42),
          CELL_SIZE * (isDeluxeStormcoilerBody ? 0.34 : 0.48),
          isDeluxeStormcoilerBody ? 1.32 : 1.3,
          34
        ),
        bodyMat
      )
    );
    superHull.position.y = isDeluxeStormcoilerBody ? 1.18 : 1.16;
    group.add(superHull);

    const superHullRing = cast(
      new THREE.Mesh(new THREE.TorusGeometry(CELL_SIZE * (isDeluxeStormcoilerBody ? 0.22 : 0.3), 0.038, 12, 42), coreMat)
    );
  ...
  const lowerCore = cast(
    new THREE.Mesh(
      new THREE.CylinderGeometry(
        isStormcoilerBody ? (isDeluxeStormcoilerBody ? 0.72 : 0.8) : 0.86,
        isStormcoilerBody ? (isDeluxeStormcoilerBody ? 0.84 : 0.92) : 0.98,
        isStormcoilerBody ? (isDeluxeStormcoilerBody ? 1.42 : 1.24) : 1.15,
        isStormcoilerBody ? 28 : 24
      ),
      bodyMat
    )
  );
  lowerCore.position.y = isStormcoilerBody ? (isDeluxeStormcoilerBody ? 1.18 : 1.08) : 0.95;
  group.add(lowerCore);

  const neck = cast(
    new THREE.Mesh(
      new THREE.CylinderGeometry(
        isStormcoilerBody ? (isDeluxeStormcoilerBody ? 0.42 : 0.46) : 0.52,
        isStormcoilerBody ? (isDeluxeStormcoilerBody ? 0.5 : 0.56) : 0.62,
        isStormcoilerBody ? (isDeluxeStormcoilerBody ? 0.42 : 0.36) : 0.3,
        24
      ),
      coreMat
    )
  );
  neck.position.y = isStormcoilerBody ? (isDeluxeStormcoilerBody ? 1.88 : 1.68) : 1.67;
  group.add(neck);

  const isBombarderBody = towerTypeId === "bombarder" || towerTypeId === "deluxeBombarder";
  if (isStormcoilerBody) {
    const postCount = isDeluxeStormcoilerBody ? 8 : 6;
    const postRadius = isDeluxeStormcoilerBody ? 1.02 : 0.88;
    const postY = isDeluxeStormcoilerBody ? 0.86 : 0.78;
    for (let i = 0; i < postCount; i += 1) {
      const angle = (i / postCount) * Math.PI * 2;
      const insulator = cast(
        new THREE.Mesh(
          new THREE.CylinderGeometry(
            isDeluxeStormcoilerBody ? 0.048 : 0.044,
            isDeluxeStormcoilerBody ? 0.056 : 0.05,
            isDeluxeStormcoilerBody ? 0.62 : 0.52,
            10
          ),
          darkMat
        )
      );
      insulator.position.set(Math.cos(angle) * postRadius, postY, Math.sin(angle) * postRadius);
      group.add(insulator);

      const insulatorCap = cast(
        new THREE.Mesh(new THREE.SphereGeometry(isDeluxeStormcoilerBody ? 0.06 : 0.055, 10, 10), glowMat)
      );
      insulatorCap.position.set(
        Math.cos(angle) * postRadius,
        postY + (isDeluxeStormcoilerBody ? 0.34 : 0.3),
        Math.sin(angle) * postRadius
      );
      group.add(insulatorCap);
    }
  } else {
  ...
  const ring = cast(
    new THREE.Mesh(
      new THREE.TorusGeometry(
        isStormcoilerBody ? (isDeluxeStormcoilerBody ? 1.08 : 0.92) : 0.96,
        isStormcoilerBody ? (isDeluxeStormcoilerBody ? 0.072 : 0.064) : 0.06,
        12,
        isStormcoilerBody ? 40 : 32
      ),
      glowMat
    )
  );
  ring.rotation.x = Math.PI / 2;
  ring.position.y = isStormcoilerBody ? (isDeluxeStormcoilerBody ? 1.64 : 1.5) : 1.46;
  group.add(ring);

  if (isStormcoilerBody) {
    const chassisRing = cast(
      new THREE.Mesh(new THREE.TorusGeometry(isDeluxeStormcoilerBody ? 0.78 : 0.66, isDeluxeStormcoilerBody ? 0.05 : 0.045, 10, 34), bodyMat)
    );
    chassisRing.rotation.x = Math.PI / 2;
    chassisRing.position.y = isDeluxeStormcoilerBody ? 1.02 : 0.9;
    group.add(chassisRing);

    const inductionPlinth = cast(
      new THREE.Mesh(
        new THREE.CylinderGeometry(
          isDeluxeStormcoilerBody ? 0.34 : 0.3,
          isDeluxeStormcoilerBody ? 0.42 : 0.36,
          isDeluxeStormcoilerBody ? 0.46 : 0.38,
          18
        ),
        coreMat
      )
    );
    inductionPlinth.position.y = isDeluxeStormcoilerBody ? 1.42 : 1.28;
    group.add(inductionPlinth);

    if (isDeluxeStormcoilerBody) {
      const containmentRing = cast(new THREE.Mesh(new THREE.TorusGeometry(CELL_SIZE * 0.5, 0.046, 12, 42), coreMat));
      containmentRing.rotation.x = Math.PI / 2;
      containmentRing.position.y = 0.56;
  ...
  if (
    ...
    const hub = cast(
      new THREE.Mesh(
        new THREE.CylinderGeometry(
          isStormcoiler ? (isDeluxeStormcoiler ? 0.44 : 0.38) : isDeluxeStormcoiler ? 0.42 : 0.34,
          isStormcoiler ? (isDeluxeStormcoiler ? 0.58 : 0.5) : isDeluxeStormcoiler ? 0.54 : 0.45,
          isStormcoiler ? (isDeluxeStormcoiler ? 0.52 : 0.44) : isDeluxeStormcoiler ? 0.78 : 0.62,
          22
        ),
        coreMat
      )
    );
    hub.position.y = isStormcoiler ? (isDeluxeStormcoiler ? 0.34 : 0.3) : 0.22;
    turret.add(hub);

    if (isStormcoiler) {
      // Dedicated Tesla profile: one high-voltage rod with rotating field rings.
      const coilCenterZ = 0;
      const rodHeight = isDeluxeStormcoiler ? 1.86 : 1.54;
      const rodCenterY = isDeluxeStormcoiler ? 1.04 : 0.9;
      spinNode = new THREE.Group();
      spinNode.position.set(0, rodCenterY + (isDeluxeStormcoiler ? 0.02 : 0), coilCenterZ);
      const rotatingRingSpecs = [
        {
          radius: isDeluxeStormcoiler ? 0.56 : 0.48,
          tube: isDeluxeStormcoiler ? 0.05 : 0.046,
          rotation: [Math.PI / 2, 0, 0],
          mat: glowMat,
        },
        {
          radius: isDeluxeStormcoiler ? 0.46 : 0.4,
          tube: isDeluxeStormcoiler ? 0.044 : 0.04,
          rotation: [0, Math.PI / 2, 0],
          mat: coreMat,
        },
        {
          radius: isDeluxeStormcoiler ? 0.36 : 0.32,
          tube: isDeluxeStormcoiler ? 0.036 : 0.033,
          rotation: [0, 0, Math.PI / 2],
          mat: glowMat,
        },
      ];
      if (isDeluxeStormcoiler) {
        rotatingRingSpecs.push({
          radius: 0.3,
          tube: 0.03,
        ...
        rotatingRingSpecs.push({
          radius: 0.24,
          tube: 0.024,
          rotation: [Math.PI / 2, -Math.PI / 5, Math.PI / 3],
          mat: glowMat,
        });
      }
      for (const spec of rotatingRingSpecs) {
        const rotatingRing = cast(new THREE.Mesh(new THREE.TorusGeometry(spec.radius, spec.tube, 10, 34), spec.mat));
        rotatingRing.rotation.set(spec.rotation[0], spec.rotation[1], spec.rotation[2]);
        spinNode.add(rotatingRing);
      }
      turret.add(spinNode);

      const centerRod = cast(
        new THREE.Mesh(
          new THREE.CylinderGeometry(
            isDeluxeStormcoiler ? 0.076 : 0.068,
            isDeluxeStormcoiler ? 0.094 : 0.084,
            rodHeight,
            12
          ),
          coreMat
      ...
      centerRod.position.set(0, rodCenterY, coilCenterZ);
      turret.add(centerRod);

      const ceramicSleeve = cast(
        new THREE.Mesh(
          new THREE.CylinderGeometry(
            isDeluxeStormcoiler ? 0.16 : 0.14,
            isDeluxeStormcoiler ? 0.2 : 0.17,
            isDeluxeStormcoiler ? 0.5 : 0.42,
            12
          ),
          bodyMat
        )
      );
      ceramicSleeve.position.set(0, rodCenterY - rodHeight * 0.28, coilCenterZ);
      turret.add(ceramicSleeve);

      const rodCollar = cast(
        new THREE.Mesh(new THREE.TorusGeometry(isDeluxeStormcoiler ? 0.24 : 0.2, isDeluxeStormcoiler ? 0.038 : 0.034, 10, 28), glowMat)
      );
      rodCollar.rotation.x = Math.PI / 2;
      rodCollar.position.set(0, rodCenterY - rodHeight * 0.02, coilCenterZ);
      turret.add(rodCollar);

      const beacon = new THREE.Mesh(new THREE.SphereGeometry(isDeluxeStormcoiler ? 0.13 : 0.11, 12, 12), beaconMat);
      beacon.position.set(0, rodCenterY + rodHeight * 0.52, coilCenterZ);
      turret.add(beacon);

      const terminalHalo = cast(
        new THREE.Mesh(new THREE.TorusGeometry(isDeluxeStormcoiler ? 0.19 : 0.16, isDeluxeStormcoiler ? 0.022 : 0.02, 10, 24), glowMat)
      );
      terminalHalo.rotation.x = Math.PI / 2;
      terminalHalo.position.set(0, rodCenterY + rodHeight * 0.46, coilCenterZ);
      turret.add(terminalHalo);
    } else {
      spinNode = new THREE.Group();
    ...
    if (towerTypeId === "volt") {
      ...
      const isDeluxeStormcoiler = towerTypeId === "deluxeStormcoiler";
      const ringCenterY = isDeluxeStormcoiler ? 1.02 : 0.9;
      const ringGeometries = [
        {
          radius: isDeluxeStormcoiler ? 0.68 : 0.58,
          tube: isDeluxeStormcoiler ? 0.054 : 0.048,
          yOffset: isDeluxeStormcoiler ? -0.34 : -0.28,
          mat: glowMat,
        },
        {
          radius: isDeluxeStormcoiler ? 0.54 : 0.46,
          tube: isDeluxeStormcoiler ? 0.046 : 0.042,
          yOffset: isDeluxeStormcoiler ? -0.08 : -0.06,
          mat: bodyMat,
        },
        {
          radius: isDeluxeStormcoiler ? 0.42 : 0.36,
          tube: isDeluxeStormcoiler ? 0.04 : 0.037,
          yOffset: isDeluxeStormcoiler ? 0.18 : 0.16,
          mat: coreMat,
        },
      ];
      for (const ring of ringGeometries) {
        const coilRing = cast(new THREE.Mesh(new THREE.TorusGeometry(ring.radius, ring.tube, 10, 34), ring.mat));
        coilRing.rotation.x = Math.PI / 2;
        coilRing.position.set(0, ringCenterY + ring.yOffset, 0);
        turret.add(coilRing);
      }

      if (isDeluxeStormcoiler) {
        const containmentBand = cast(new THREE.Mesh(new THREE.TorusGeometry(0.8, 0.032, 10, 36), coreMat));
        containmentBand.rotation.x = Math.PI / 2;
        containmentBand.position.set(0, ringCenterY - 0.18, 0);
        turret.add(containmentBand);

        const crownRing = cast(new THREE.Mesh(new THREE.TorusGeometry(0.48, 0.03, 10, 30), glowMat));
        crownRing.rotation.set(Math.PI / 2, Math.PI / 5, 0);
        crownRing.position.set(0, ringCenterY + 0.42, 0);
        turret.add(crownRing);

        const crownRingB = cast(new THREE.Mesh(new THREE.TorusGeometry(0.34, 0.024, 10, 28), glowMat));
        crownRingB.rotation.set(Math.PI / 2, 0, Math.PI / 3);
        crownRingB.position.set(0, ringCenterY + 0.56, 0);
        turret.add(crownRingB);
      } else {
        const arcLoop = cast(new THREE.Mesh(new THREE.TorusGeometry(0.26, 0.024, 10, 24), glowMat));
        arcLoop.rotation.set(Math.PI / 2, Math.PI / 6, 0);
        arcLoop.position.set(0, ringCenterY + 0.36, 0);
        turret.add(arcLoop);
      }
    ...
    if (towerTypeId === "tesla") muzzle.position.set(0, 1.72, 0);
    if (towerTypeId === "deluxeStormcoiler") muzzle.position.set(0, 2.06, 0);
...
class BombardMuzzleFireEffect {
  constructor(origin, direction, color, radius) {
    this.age = 0;
    this.life = 0.3;
    this.baseSize = Math.max(0.14, (Number(radius) || 0.08) * 1.8);
    this.origin = origin.clone();
    this.direction = direction.clone();
    if (this.direction.lengthSq() > 1e-6) this.direction.normalize();
    else this.direction.set(0, 0, 1);

    const flameColor = new THREE.Color(color).lerp(new THREE.Color("#ff8f2b"), 0.56);
    const hotColor = flameColor.clone().lerp(new THREE.Color("#fff3cc"), 0.62);
    const emberColor = new THREE.Color("#ffb24d");

    this.group = new THREE.Group();
    this.group.position.copy(this.origin).addScaledVector(this.direction, this.baseSize * 0.26);
    this.group.quaternion.setFromUnitVectors(PROJECTILE_FORWARD, this.direction);
    ...
    this.outerFlame = new THREE.Mesh(
      new THREE.ConeGeometry(this.baseSize * 1.24, this.baseSize * 5.8, 16, 1, true),
      this.outerMat
    );
    this.outerFlame.rotation.x = Math.PI / 2;
    this.outerFlame.position.z = this.baseSize * 1.84;
    this.group.add(this.outerFlame);

    this.midMat = new THREE.MeshBasicMaterial({
      color: flameColor.clone().lerp(new THREE.Color("#ffbe5c"), 0.34),
      transparent: true,
      opacity: 0.92,
      side: THREE.DoubleSide,
      blending: THREE.AdditiveBlending,
    ...
    this.midFlame = new THREE.Mesh(
      new THREE.ConeGeometry(this.baseSize * 0.94, this.baseSize * 4.4, 14, 1, true),
      this.midMat
    );
    this.midFlame.rotation.x = Math.PI / 2;
    this.midFlame.position.z = this.baseSize * 1.38;
    this.group.add(this.midFlame);
    ...
    this.innerFlame = new THREE.Mesh(
      new THREE.ConeGeometry(this.baseSize * 0.56, this.baseSize * 2.8, 12, 1, true),
      this.innerMat
    );
    this.innerFlame.rotation.x = Math.PI / 2;
    this.innerFlame.position.z = this.baseSize * 1.02;
    this.group.add(this.innerFlame);
    ...
    this.core = new THREE.Mesh(new THREE.SphereGeometry(this.baseSize * 0.96, 12, 12), this.coreMat);
    this.core.position.z = this.baseSize * 0.32;
    this.group.add(this.core);
    ...
    this.ring = new THREE.Mesh(
      new THREE.RingGeometry(this.baseSize * 0.56, this.baseSize * 1.2, 24),
      this.ringMat
    );
    this.ring.position.z = this.baseSize * 0.2;
    this.group.add(this.ring);

    this.crossRingMat = new THREE.MeshBasicMaterial({
      color: emberColor,
      transparent: true,
      opacity: 0.7,
      side: THREE.DoubleSide,
    ...
    this.crossRing = new THREE.Mesh(
      new THREE.RingGeometry(this.baseSize * 0.48, this.baseSize * 0.92, 20),
      this.crossRingMat
    );
    this.crossRing.rotation.x = Math.PI / 2;
    this.crossRing.position.z = this.baseSize * 0.16;
    this.group.add(this.crossRing);
    ...
    for (let i = 0; i < 10; i += 1) {
      const spark = new THREE.Mesh(
        new THREE.SphereGeometry(this.baseSize * (0.08 + Math.random() * 0.04), 6, 6),
        this.sparkMat
      );
      spark.position.set(
        (Math.random() - 0.5) * this.baseSize * 0.34,
        (Math.random() - 0.5) * this.baseSize * 0.28,
        this.baseSize * (0.1 + Math.random() * 0.24)
      );
    ...
    this.light = new THREE.PointLight(hotColor, 5.8, 11.5, 2);
    this.light.position.set(0, 0, this.baseSize * 0.28);
  ...
  update(dt) {
    this.age += dt;
    const t = THREE.MathUtils.clamp(this.age / this.life, 0, 1);
    const flare = 1 + Math.sin(this.age * 96) * 0.1;

    this.group.position.addScaledVector(this.direction, dt * (8.2 + this.baseSize * 10.4));
    this.outerFlame.scale.set((1 + t * 2.6) * flare, (1 + t * 2.6) * flare, 1 + t * 1.46);
    this.midFlame.scale.set((1 + t * 2.1) * flare, (1 + t * 2.1) * flare, 1 + t * 1.28);
    this.innerFlame.scale.set(1 + t * 1.5, 1 + t * 1.5, 1 + t * 1.1);
    this.core.scale.setScalar((1 + t * 1.42) * flare);
    this.ring.scale.set(1 + t * 3.0, 1 + t * 3.0, 1);
    this.crossRing.scale.set(1 + t * 2.5, 1 + t * 2.5, 1);

    for (const spark of this.sparks) {
      spark.mesh.position.addScaledVector(spark.velocity, dt);
      spark.velocity.x *= Math.max(0, 1 - dt * 4.2);
      spark.velocity.y *= Math.max(0, 1 - dt * 4.2);
      spark.velocity.z *= Math.max(0, 1 - dt * 3.6);
      spark.velocity.y += dt * 0.24;
      spark.mesh.scale.setScalar(Math.max(0.2, 1 - t * 0.9));
    }

    this.outerMat.opacity = Math.max(0, 0.98 * (1 - t * 1.22));
    this.midMat.opacity = Math.max(0, 0.92 * (1 - t * 1.26));
    this.innerMat.opacity = Math.max(0, 0.88 * (1 - t * 1.3));
    this.coreMat.opacity = Math.max(0, 0.9 * (1 - t * 1.18));
    this.ringMat.opacity = Math.max(0, 0.78 * (1 - t * 1.14));
    this.crossRingMat.opacity = Math.max(0, 0.7 * (1 - t * 1.2));
    this.sparkMat.opacity = Math.max(0, 0.92 * (1 - t * 1.08));
    this.light.intensity = Math.max(0, (5.8 + Math.sin(this.age * 122) * 0.8) * (1 - t * 0.7));
    return t < 1;
  }
    ...
    fragments.push(`
      <div class="menu-account-item">
        <div>
          ...
          <span>Create with username + email, or login with email, to sync this account across devices.</span>
        </div>
      </div>
    `);
  }
...
function createAccountFromInput() {
  ...
  const rawDisplayName = accountCreateDisplayNameInputEl ? accountCreateDisplayNameInputEl.value : "";
  const displayName = sanitizeAccountName(rawDisplayName || "", ACCOUNT_DISPLAY_NAME_MAX_LENGTH);
  const rawEmail = accountCreateUsernameInputEl ? accountCreateUsernameInputEl.value : "";
  ...
  if (!displayName) {
    setAccountStatus("Enter a username first.", true);
    return;
  }
  if (!email) {
...
function loginAccountFromInput() {
  ...
  const rawEmail = accountLoginUsernameInputEl ? accountLoginUsernameInputEl.value : "";
  const email = sanitizeAccountUsername(rawEmail);
...
function renderMenuShop() {
  ...
  for (const towerTypeId of MENU_UNLOCK_TOWER_IDS) {
    ...
    const portraitDataUrl = getTowerCardPortraitDataUrl(towerTypeId);
    const hasPortrait = !!portraitDataUrl;
    const accentA = colorHexToRgbaCss(type.bodyColor, 0.3);
    const accentB = colorHexToRgbaCss(type.coreColor, 0.24);
    const edgeGlow = colorHexToRgbaCss(type.bodyColor, 0.68);
...
function ensureCreatureCardPortraitRenderer() {
  if (
    ...
    creatureCardPortraitSubject &&
    creatureCardPortraitCanvas
  ) {
    return true;
  }
  ...
  canvasEl.height = CREATURE_CARD_PORTRAIT_SIZE;

  const portraitRenderer = tryCreateWebglRenderer({
    canvas: canvasEl,
    alpha: true,
    antialias: true,
    powerPreference: "low-power",
  ...
  if (!portraitRenderer) return false;
...
function disposeCreaturePortraitObject(object3d) {
  if (!object3d) return;
...
function clearCreaturePortraitSubject() {
  if (!creatureCardPortraitSubject) return;
  while (creatureCardPortraitSubject.children.length > 0) {
    const child = creatureCardPortraitSubject.children[creatureCardPortraitSubject.children.length - 1];
    creatureCardPortraitSubject.remove(child);
    disposeCreaturePortraitObject(child);
  }
}
...
function tuneTowerPortraitMeshReadability(group, towerType) {
  ...
  const emissiveTint = primary.clone().lerp(secondary, 0.2);
  ...
  group.traverse((node) => {
    ...
    for (const material of materials) {
      ...
      if (material.isMeshPhysicalMaterial) {
        ...
        material.opacity = 1;
        if (Number.isFinite(material.transmission)) {
          material.transmission = Math.max(0.14, Math.min(0.56, material.transmission || 0.26));
        }
        if (Number.isFinite(material.thickness)) {
          material.thickness = Math.max(0.14, Math.min(0.72, material.thickness || 0.3));
        }
        if (Number.isFinite(material.roughness)) {
          material.roughness = Math.max(0.06, Math.min(0.28, material.roughness || 0.16));
        }
        if (Number.isFinite(material.clearcoat)) {
          material.clearcoat = Math.max(0.46, Math.min(1, material.clearcoat || 0.72));
        }
        if (Number.isFinite(material.clearcoatRoughness)) {
          material.clearcoatRoughness = Math.max(0.02, Math.min(0.16, material.clearcoatRoughness || 0.09));
        }
        if (Number.isFinite(material.ior)) material.ior = Math.max(1.22, Math.min(1.52, material.ior || 1.34));
        if (Number.isFinite(material.envMapIntensity)) {
          material.envMapIntensity = Math.max(1.02, Math.min(1.55, material.envMapIntensity || 1.16));
        }
        if (material.emissive?.isColor) {
          material.emissive.lerp(emissiveTint, 0.02);
          material.emissiveIntensity = Math.min(0.12, Math.max(0.01, material.emissiveIntensity || 0.04));
        }
        material.needsUpdate = true;
      } else if (material.isMeshStandardMaterial) {
        material.transparent = false;
        material.opacity = 1;
        if (Number.isFinite(material.roughness)) {
          material.roughness = Math.max(0.14, Math.min(0.42, material.roughness || 0.26));
        }
        if (Number.isFinite(material.metalness)) {
          material.metalness = Math.max(0.08, Math.min(0.38, material.metalness || 0.16));
        }
        if (material.emissive?.isColor) {
          material.emissive.lerp(emissiveTint, 0.015);
          material.emissiveIntensity = Math.min(0.18, Math.max(0.01, material.emissiveIntensity || 0.04));
        }
        material.needsUpdate = true;
      }
    }
...
function renderCreatureCardPortraitDataUrl(enemyTypeId) {
  ...
  try {
    ...
    if (enemyVisual?.group) {
      creatureCardPortraitSubject.remove(enemyVisual.group);
      disposeCreaturePortraitObject(enemyVisual.group);
    }
    return "";
  }
}
...
function renderCreatureShop() {
  ...
  for (const enemyTypeId of CREATURE_SHOP_ENEMY_IDS) {
    ...
    const accentB = colorHexToRgbaCss(enemyType.colorB, 0.38);
    const edgeGlow = colorHexToRgbaCss(enemyType.colorA, 0.82);
    ...
    const classes = [
      ...
      .join(" ");
    const portraitMarkup = hasPortrait
      ? `<div class="menu-creature-portrait" aria-hidden="true" style="background-image: url('${portraitDataUrl}')"></div>`
      : "";
...
function updateHud() {
  ...
  if (openUnlockFromCreatureBtn) openUnlockFromCreatureBtn.disabled = lockMetaMenusDuringRun;
  if (openTrapFromCreatureBtn) openTrapFromCreatureBtn.disabled = lockMetaMenusDuringRun;
  if (openUnlockFromTrapBtn) openUnlockFromTrapBtn.disabled = lockMetaMenusDuringRun;
  if (openCreatureFromTrapBtn) openCreatureFromTrapBtn.disabled = lockMetaMenusDuringRun;
  if (openGameUpgradesBtn) openGameUpgradesBtn.disabled = lockMetaMenusDuringRun;
...
function updateMapEffects(dt) {
  ...
  if (mapState.starField) {
    ...
    if (marsLevel && mapState.starField.material) {
      const dustPulse = 0.82 + Math.sin(game.time * 0.72) * 0.16;
      mapState.starField.material.opacity = Math.max(0.12, 0.28 * dustPulse);
    }
  }

  if (marsLevel) {
    tealRim.intensity = 0.38 + Math.sin(game.time * 1.18) * 0.06;
    emberRim.intensity = 0.36 + Math.sin(game.time * 1.02 + 1.2) * 0.05;
    prismKeyLight.intensity = 0.3 + Math.sin(game.time * 1.28 + 0.4) * 0.06;
    prismFillLight.intensity = 0.24 + Math.sin(game.time * 1.12 + 1.9) * 0.05;
  ...
  if (mapState.mapLights.length >= 3) {
    if (marsLevel) {
      mapState.mapLights[0].intensity = 0.22 + Math.sin(game.time * 1 + 0.2) * 0.05;
      mapState.mapLights[1].intensity = 0.2 + Math.sin(game.time * 0.94 + 1.6) * 0.04;
      mapState.mapLights[2].intensity = 0.12 + Math.sin(game.time * 1.08 + 2.2) * 0.03;
    } else if (emberLevel) {
...
if (openUnlockFromCreatureBtn) openUnlockFromCreatureBtn.addEventListener("click", openUnlocksMenu);
if (openTrapFromCreatureBtn) openTrapFromCreatureBtn.addEventListener("click", openTrapMenu);
if (openUnlockFromTrapBtn) openUnlockFromTrapBtn.addEventListener("click", openUnlocksMenu);
if (openCreatureFromTrapBtn) openCreatureFromTrapBtn.addEventListener("click", openCreatureMenu);
...
for (const inputEl of [accountCreateDisplayNameInputEl, accountCreateUsernameInputEl, accountCreatePasswordInputEl, accountCreatePasswordConfirmInputEl]) {
  if (!inputEl) continue;
...
window.addEventListener("keydown", (event) => {
  ...
  if (
    (accountCreateDisplayNameInputEl && document.activeElement === accountCreateDisplayNameInputEl) ||
